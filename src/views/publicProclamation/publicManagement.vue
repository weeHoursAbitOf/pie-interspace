<template>
  ...
  <div class="pie-tb-content">
        ...
        <div class="pie-tb-header">
          <div class="tb-header-content">
            ...
            <div class="btn-content">
              ...
              <batch-delete-btn class="right-btn-item" @click="batchesDelete"></batch-delete-btn>
              <new-btn title="加入" class="right-btn-item" @click="add"></new-btn>
            </div>
          </div>
        </div>
      </div>
    ...
    >
      <div slot="operateItem">
        <div class="modal-title">
          ...
          <div class="modal-header">
            <span>描述</span>
            <Input v-model="description" class="input-style" />
          </div>
          <div class="modal-header">
            <look-btn title="预览" @click="preview" class="text-style"></look-btn>
          </div>
        </div>
    ...
    <!-- 删除弹出框 -->
    <my-dialog @confirm="confirmDel" @cancel="cancel" :isModal="confirmDelete"></my-dialog>
  </div>
</template>
<script>
  ...
  methods: {
    ...
    confirmAdd() {
      ...
      if (this.addOrUpdate == 0) {
        ...
        this.$api.regulationManage.updateInfo(this.indexName, params).then((res) => {
          if (res.code == 0) {
            this.isModal = false;
            this.$Notice.success({
              title: res.message,
