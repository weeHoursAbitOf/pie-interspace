...
export default {
  ...
  watch: {
    $route(to) {
      ...
      this.indexName = to.query.typeName;
      this.getData();
    },
  },
  created() {
    this.routerPath = this.$router.query.path;
    this.routerName = this.$router.query.name;
    this.indexName = this.$router.query.typeName;
    this.getData();
  },
  methods: {
    //查询数据
        ...
        if (res.code == 0) {
          for (let i = 0; i < res.data.msg.length; i++) {
            if (res.data.msg[i].status == 0) {
              res.data.msg[i].status = "删除";
            } else if (res.data.msg[i].status == 1) {
              res.data.msg[i].status = "草稿";
            } else if (res.data.msg[i].status == 2) {
              res.data.msg[i].status = "发布";
            }
          }
    ...
    topMove(row) {
      if (row.top == 0) {
        ...
        this.$api.regulationManage.offTopInfo(params).then((res) => {
          if (res.code == 0) {
            this.$Notice.success({
              title: res.message,
              top: 50,
