...
export default {
  ...
  methods: {
    ...
    getUnpublishedData() {
      ...
      this.$api.statement.getApproachList(params).then((res) => {
        if (res.success == true) {
    ...
    moversConfirmDelete(row) {
      ...
      this.$api.statement.getBatchDelete(params).then((res) => {
        if (res.success == true) {
          this.$Notice.success({
            title: res.message,
            top: 50,
    ...
    // 下载
    download(row) {
      this.$api.statement.getApproachDownload(row.id).then((res) => {
