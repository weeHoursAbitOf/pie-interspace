...
export default {
  ...
  methods: {
    ...
    getUnpublishedData() {
      ...
      this.$api.statement.advancedFall(formData).then((res) => {
        if (res.success == true) {
    ...
    confirmAddition() {
      ...
      this.$api.statement.advancedFallProduce(params).then((res) => {
        if (res.success == true) {
          this.getUnpublishedData(); //未发布数据列表
          this.getReleasedData(); //已发布数据列表
          this.getAllDataList(); //获取全部数据列表
    ...
    confirmDelete() {
      ...
      this.$api.statement.DeleteFall(params).then((res) => {
        if (res.success == true) {
          this.$Notice.success({
            title: res.message,
            top: 50,
