  ...
  methods: {
    ...
    getUnpublishedData() {
      let params = {
        ...
        startTime: this.unCreateTime,
        endTime: this.unEndTime,
        serialNum: this.unSerialNumber,
      };
      this.$api.statement.getCollisionWarningList(params).then((res) => {
        if (res.success == true) {
    ...
    getReleasedData() {
      let params = {
        ...
        startTime: this.finishCreateTime,
    ...
    moversConfirmDelete(row) {
      ...
      this.$api.statement.getCollisionWarningDelete(params).then((res) => {
        if (res.success == true) {
          this.$Notice.success({
            title: res.message,
            top: 50,
    ...
    // 下载
    download(row) {
      this.$api.statement.getCollisionWarningDownload(row.id).then((res) => {
