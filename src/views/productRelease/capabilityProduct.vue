...
export default {
  ...
  data() {
    return {
      ...
      moversData: [
        ...
        //   serialNum: "1",
        //   storageName: "",
        //   imgUrl: require("@/assets/public/capabilityProduct.svg"),
        // },
      ],
      // 未发布------------
  ...
  methods: {
    ...
    getUnpublishedData() {
      ...
      this.$api.systemAppoint.systemCapacity(params).then((res) => {
        if (res.success == true) {
    ...
    //公共确认删除按钮
    confirmDelete() {
      this.$api.systemAppoint
        .DeleteCapacity(this.pitchOnItem.id)
        .then((res) => {
          if (res.success == true) {
            this.$Notice.success({
              title: res.message,
