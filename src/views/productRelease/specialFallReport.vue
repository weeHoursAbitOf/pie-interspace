...
export default {
  ...
  methods: {
    ...
    moversConfirmDelete(row) {
      this.pitchOnItem = row;
      let formData = new FormData();
      formData.append("ids", this.pitchOnItem.id);
      this.$api.statement.DeleteDelallFall(formData).then((res) => {
        if (res.success == true) {
          this.$Notice.success({
            title: res.message,
            top: 50,
    ...
    download(row) {
      ...
      this.$api.statement.downloadFallReport(params).then((res) => {
    ...
    confirmDelete() {
      let formData = new FormData();
      formData.append("ids", this.selectRow.map((item) => item.id).join(","));
      this.$api.statement.DeleteDelallFall(formData).then((res) => {
        if (res.success == true) {
          this.$Notice.success({
            title: res.message,
