<template>
  ...
  <div class="pie-tb-content">
      ...
      <div slot="header">
        <div class="publish-module-content">
          <div class="publish-module-message">
            ...
            <div>
              <span>期刊号</span>
              <Input v-model="serialNumber" placeholder="请输入期刊号" style="width: 250px" />
            </div>
            <div class="publish-module-btn">
              <search-btn class="btn-right" @click="moversSolSearch"></search-btn>
              <batch-delete-btn class="btn-right" @click="moversSolDelete"></batch-delete-btn>
              <batch-join-btn class="btn-right" @click="additionProductEvent"></batch-join-btn>
              <div class="iconfont icon-tongji btn-right" @click="switchElse"></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="publish-module-content">
        <div class="publish-module-top">
          ...
          <div class="publish-module-message">
            ...
            <div>
              <span>期刊号</span>
              <Input v-model="unSerialNumber" placeholder="请输入期刊号" style="width: 250px" />
            </div>
          </div>
          <div class="publish-module-btn">
        ...
        <div class="publish-module-message">
          <div class="radio-messag-image">
            <RadioGroup :value="unPitchOnItem" @on-change="onUnPadioCange" class="radio-event">
      ...
      <div class="publish-module-content" style="margin-top: 10px">
        <div class="publish-module-top">
          ...
          <div class="publish-module-message">
            ...
            <div>
              <span>期刊号</span>
              <Input v-model="finishSerialNumber" placeholder="请输入期刊号" style="width: 250px" />
            </div>
          </div>
          <div class="publish-module-btn">
        ...
        <div class="publish-module-message">
          <div class="radio-messag-image">
            <RadioGroup
              ...
              class="radio-event"
            >
              <Radio v-for="item in finishDataList" :key="item.id" :label="item">
                <img class="messag-img" :src="item.imgUrl" alt />
              </Radio>
            </RadioGroup>
          </div>
    ...
    >
      <div slot="operateItem">
        <uploadFileMoyle :resetParams="resetParams" @upload-file-event="uploadFileEvent" />
      </div>
    </my-modal>
    <!-- 删除弹出框 -->
    ...
    <!-- 提交审核弹出框 -->
    <my-dialog @confirm="confirmDelete" @cancel="cancel" :isModal="isSubmit" content="确定要提交吗吗?"></my-dialog>
  </div>
</template>
<script>
...
export default {
  ...
  data() {
    return {
      ...
      moversData: [
        ...
        //   serialNum: "1",
        //   storageName: "",
        //   imgUrl: require("@/assets/public/issue.png"),
        // },
      ],
      // 未发布------------
      ...
      versionData: [
        ...
        //   id: 0,
        //   version: "1.0",
        //   productPath: "发射产品.doc",
        //   auditor: "7302",
        //   createTime: "2021-20-12 03:03:03",
        //   description: "sadsadsadsadsad",
        //   productId: 0,
        //   productIndexTableName: "202107",
        //   updateTime: "2002-20-20 00:00:00",
        // },
        // {
        //   id: 1,
        //   version: "1.0",
  ...
  methods: {
    ...
    getUnpublishedData() {
      ...
      // formData.append("status", 0); //未发布状态
      // formData.append("page", this.unPageNum); //页数
      // formData.append("size", this.unPageSize); //条数
      // formData.append("beginTime", this.unCreateTime); //开始时间
      // formData.append("endTime", this.unEndTime); //结束时间
      // formData.append("serialNum", this.unSerialNumber); //期刊号
      let params = {
        status: 0,
        pageNum: this.unPageNum,
        pageSize: this.unPageSize,
        createTime: this.unCreateTime,
        endTime: this.unEndTime,
        serialNum: this.unSerialNumber
      }
      this.$api.statement.spaceDebris(params).then((res) => {
        if (res.success == true) {
    ...
    getReleasedData() {
      // let formData = new FormData();
      // formData.append("status", 1); //未发布状态
      // formData.append("page", this.finishPageNum); //页数
      // formData.append("size", this.finishPageSize); //条数
      // formData.append("beginTime", this.finishCreateTime); //开始时间
      // formData.append("endTime", this.finishEndTime); //结束时间
      // formData.append("serialNum", this.finishSerialNumber); //期刊号
      let params = {
        status: 1,
        pageNum: this.finishPageNum,
        pageSize: this.finishPageSize,
        createTime: this.finishCreateTime,
    ...
    getAllDataList() {
      // let formData = new FormData();
      // formData.append("status", ""); //未发布状态
      // formData.append("page", this.pageNum); //页数
      // formData.append("size", this.pageSize); //条数
      // formData.append("beginTime", this.createTime); //开始时间
      // formData.append("endTime", this.endTime); //开始时间
      // formData.append("serialNum", this.serialNumber); //期刊号
    ...
    confirmDelete() {
      let params = {
        id: this.pitchOnItem.id,
      };
      this.$api.statement.DeleteSpaceDebris(params, this.pitchOnItem.id).then((res) => {
        if (res.success == true) {
          this.$Notice.success({
            title: res.message,
            top: 50,
