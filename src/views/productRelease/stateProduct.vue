...
export default {
  ...
  data() {
    return {
      ...
      moversData: [
        ...
        //   serialNum: "1",
        //   storageName: "",
        //   imgUrl: require("@/assets/public/stateProduct.svg"),
        // },
      ],
      // 未发布------------
  ...
  methods: {
    ...
    getUnpublishedData() {
      let params = {
        status: 0, //发布状态
        pageNum: this.unPageNum, //页数
        pageSize: this.unPageSize, //条数
        createTime: this.unCreateTime, //开始时间
        endTime: this.unEndTime, //结束时间
        serialNum: this.unSerialNumber, //期刊号
      };
      this.$api.systemAppoint.systemStatus(params).then((res) => {
        if (res.success == true) {
    ...
    getReleasedData() {
      let params = {
        status: 1, //发布状态
        pageNum: this.finishPageNum, //页数
        pageSize: this.finishPageSize, //条数
        createTime: this.finishCreateTime, //开始时间
        endTime: this.finishEndTime, //结束时间
        serialNum: this.finishSerialNumber, //期刊号
      };
      this.$api.systemAppoint.systemStatus(params).then((res) => {
        if (res.success == true) {
    ...
    getAllDataList() {
      let params = {
        ...
        createTime: this.createTime, //开始时间
        endTime: this.endTime, //结束时间
        serialNum: this.serialNumber, //期刊号
      };
      this.$api.systemAppoint.systemStatus(params).then((res) => {
        if (res.success == true) {
    ...
    //公共确认删除按钮
    confirmDelete() {
      this.$api.systemAppoint.DeleteStatus(this.pitchOnItem.id).then((res) => {
        if (res.success == true) {
          this.$Notice.success({
            title: res.message,
            top: 50,
