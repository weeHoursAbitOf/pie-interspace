...
export default {
  ...
  methods: {
    ...
    getSelUsvEwFallTime() {
      ...
      this.$api.advanced.selUsvEwFallTime(params).then((res) => {
        if (res.success == true) {
    ...
    popUpOnlineEvent(row) {
      ...
      this.$api.advanced.PushOrClose(formData).then((res) => {
        if (res.success == true) {
          this.$Notice.success({
            title: res.message,
            top: 50,
    ...
    moversConfirmDelete(row) {
      let params = [row.id];
      this.$api.advanced.closeDelete(params).then((res) => {
        if (res.success == true) {
          this.$Notice.success({
            title: res.message,
            top: 50,
