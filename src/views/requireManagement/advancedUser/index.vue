    ...
    >
      <div slot="operateItem">
        <launchModule
          ...
          @on-currentid-change="onCurrentidChange"
        />
      </div>
    </my-modal>
    <!--人工处理安全规避 -->
...
import launchModule from "./launchModule";
export default {
  data() {
    return {
      pageTotal: 10, //数据总数
  ...
  created() {
    this.getSeniorSelSafeApply();
  },
  methods: {
    // 获取数据列表
    getSeniorSelSafeApply() {
      ...
      this.$api.advanced.seniorSelSafeApply(formData).then((res) => {
        if (res.success == true) {
